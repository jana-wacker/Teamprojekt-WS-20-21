...
"""
def matchCount(data):
    count = homeAndguest(data).shape[0]
    if count>0:
        return count
    else:
        return 0
...
def scoreIntercept(data):
    intercept=predicateScore(data).intercept_
    return intercept[0]
#print(scoreIntercept(data))

def scoreCoefhomeoraway(data):
    coef_home_or_away=predicateScore(data).coef_[0][0]
    return coef_home_or_away
#print(scoreCoefhomeoraway(data))

def scoreCoefavg(data):
    coef_2_score_avg=predicateScore(data).coef_[0][1]
    return coef_2_score_avg
#print(scoreCoefavg(data))

def homeScore(data):
    df=score(data)
    homeScorePre=scoreIntercept(data)+scoreCoefhomeoraway(data)*1+scoreCoefavg(data)*df['2_score_avg'].values[-1]
    if homeScorePre<0:
        homeScorePre=0
    return homeScorePre
...
def awayScore(data):
    df=score(data)
    awayScorePre=scoreIntercept(data)+scoreCoefhomeoraway(data)*0+scoreCoefavg(data)*df['2_score_avg'].values[-1]
    if awayScorePre<0:
        awayScorePre=0
    return awayScorePre
...
def percentageIntercept(data):
    intercept = predicatePercentage(data).intercept_
    return intercept[0]
#print(percentageIntercept(data))

def percentageCoefhomeoraway(data):
    coef_home_or_away = predicatePercentage(data).coef_[0][0]
    return coef_home_or_away
#print(percentageCoefhomeoraway(data))

def percentageCoefavg(data):
    coef_2_percentage_avg = predicatePercentage(data).coef_[0][1]
    return coef_2_percentage_avg
#print(percentageCoefavg(data))

def homePercentage(data):
    df = percentage(data)
    homePercentagePre = percentageIntercept(data) + percentageCoefhomeoraway(data) * 1 + percentageCoefavg(data) * df['2_percentage_avg'].values[-1]
...
def awayPercentage(data):
    df = percentage(data)
    awayPercentagePre = percentageIntercept(data) + percentageCoefhomeoraway(data) * 0 + percentageCoefavg(data) * df['2_percentage_avg'].values[-1]
...
def tiedPrecentage(data):
    tiedPre=1-homePercentage(data)-awayPercentage(data)
    if tiedPre<0:
        tiedPre=0
    return tiedPre
...
def predict(data):
    final=homeAndguest(data)
    if(matchCount(final))==0:
        return("Sorry, the data is incomplete and cannot be used for prediction!Please choose other teams or time!")
    else:
        result = {'Home Team': homeName,
                  'Away Team': guestName,
                  'Home team win ratio': homePercentage(final),
                  'Home and away team tie ratio': tiedPrecentage(final),
                  'Away team win ratio': awayPercentage(final),
                  'Home team score': homeScore(final),
                  'Away team score': awayScore(final)}
        return result
print(predict(data))
###############################################################################################
